...
namespace {
	...
	*/
	...
	// Tests that the Foo::Bar() method does Abc.
	TEST( RingBufferTest,  Force )
	{
		constexpr unsigned TEST_SIZE = 1000;
		RingBuffer<float> rb(TEST_SIZE);
		...
		// Refill buffer
		for( int i=0; i<TEST_SIZE ; ++i )
			EXPECT_EQ( rb.pushForce(i*2), true );
		
		// Check refilled popping value back
		for( int i=0; i<TEST_SIZE ; ++i ){
			auto v = rb.pop();
			EXPECT_EQ( v, i*2 );
		}
