...
namespace Sledge {
	...
	private:  // == УПРАВЛЕНИЕ ИНДЕКСАМИ ==  //todo remove and use RingIndex
		...
		/// idx + 1
		Index_t nextIndexOf( Index_t const& idx) const 
		{
		...
		/// idx + inc
		Index_t incIndexOf( Index_t const& idx, Index_t const& inc ) const 
		{
			Index_t ret = idx + inc;
			if( ret >= bourn_ )
				ret -= bourn_;
	...
	#define BE_FASTER_OPTIMIZE  true
		...
		/** Запись в буфер массива элементов, выталкивая все предыдущие если буффер заполнен.
		  ...
		  */
		Index_t pushForce( Data_t const array[], Index_t length )
		{
			Index_t i=0;
			while( i<length ){
				pushForce( array[i++] );
